function stats = henitd( h, int, dtext, haL, haR, oneLine, tb, bSphere, nGrid )
% henitd - display HRIR energy (IID) and ITD simultaneously.
%
% stats = henitd( h, int, dtext, haL, haR, oneLine, tb, bSphere, nGrid )
%
...
if nargin < 9, nGrid = -1; end;

if h.finc == 0,
  disp( 'henitd error: requires fixed-inc grid.' );
  return;
end;
...
ziR = zeros( length(elmap), length(azmap) );
...
cax = [ -32 32 ];  % based on ITD

% see grids() for Listen/CIPIC measurement grids
if bSphere,
  % last 3 params = show bottom, show line, show grid
  % (0,1,2,3 slab3d/CIPIC/Listen/ACD)
  lrsphere( azmap, elmap, zR-zL, ziR, 'R-L IID', ...
            'ITD', cax, tb, 1, 1, nGrid );
else
  lrsurf( azmap, elmap, zR-zL, ziR, 'R-L IID', ...
          'ITD', cax, tb, int, haL, haR );
end;

%henitd(s,1,0,[],[],0,1,1);
%print(gcf,'-dpng','henitd_irc_1002.png');
